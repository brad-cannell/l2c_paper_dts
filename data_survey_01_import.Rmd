...
This file is used to import the Link2Care follow-up visit survey data and do some initial data cleaning (e.g., create calculated variables). This file is a copy of the file that was originally created in the link2care_private repository.
...
# 2021-11-04: v1 imported with 324 rows and 810 columns.
# 2021-11-04: v2 imported with 299 rows and 213 columns.
# 2021-11-04: v3 imported with 214 rows and 542 columns.
# 2021-11-04: v4 imported with 155 rows and 598 columns.
# 2021-11-04: v5 imported with 137 rows and 649 columns.
```
...
# 2021-11-04: REDCap imported with 24 rows and 3 columns.
```
...
# 2021-11-04: Master log screened in imported with 326 rows and 41 columns.
```
...
### Coerce QDS group variable to character

Currently, group is a numeric variable in QDS. We will convert it to character so that (1) it's easier to read, and (2) it's easier to check against the Master Log group values in the step below. 
...
check_qds_ml_group_diffs <- master_log_screened_in %>%
  left_join(group_qds, by = "id", suffix = c("_ml", "_qds")) %>% 
  rowwise() %>% 
  filter(length(unique(c_across(c(group_ml, group_qds)))) > 1) %>% 
...
# 2021-11-04: 327 participants have been randomized to a treatment group.
```
...
rm(
  ...
  check_redcap_group_values, group_qds_long, master_log_screened_in, redcap, 
  n_randomized
)
```
...
all_visits <- all_visits %>% 
  mutate(id = as.character(SUBJECT)) %>% 
  left_join(group_qds, by = "id")
```
...
We will use participant id a lot for data cleaning and analysis. Let's make it the first column in the data.

```{r}
all_visits <- select(all_visits, id, group, everything())
```
...
```{r}
ncol(all_visits) # 2782 So, 2810 - 2782 = 28 columns dropped.
```
...
### 25. Aggression Questionniare (AQ)
...
# VARIABLE LABELS 
attr(all_visits$PA, "label") <- 'Physical aggression subscale'
attr(all_visits$VA, "label") <- 'Verbal aggression subscale'
...
attr(all_visits$aggression_total, "label") <- 'Aggression questionnaire total'
```
...
```{r}
readr::write_rds(
  all_visits, "/Volumes/Link2Care/Statistical Projects/Robison - Distress Tolerance Scale Paper/all_visits_import.rds"
)
```
